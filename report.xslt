<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <xsl:template match="marc:collection" name="report">
                ...
                <xsl:for-each-group
                        select="marc:record/marc:datafield[@tag = ('100', '110', '111', '600', '610', '611', '700', '710', '711') and not(marc:subfield[@code='t']) and count(marc:subfield[@code = '1']) = 1 and starts-with(normalize-space(marc:subfield[@code = '1']), 'http')]"
                ...
                <xsl:if test="not(marc:record/marc:datafield[@tag = ('100', '110', '111', '600', '610', '611', '700', '710', '711') and not(marc:subfield[@code='t']) and marc:subfield[@code='a'] and marc:subfield[@code = '1']])">
                        <xsl:copy-of select="bib:list('Finner ingen agenter  identifisert med URI i $1', $format)"/>
                </xsl:if> 
                 
                <!-- Leter etter felter med agenter uten URI -->
                <xsl:variable name="missingagents" select="marc:record/marc:datafield[@tag = ('100', '110', '111', '600', '610', '611', '700', '710', '711') and not(marc:subfield[@code='t']) and not(marc:subfield/@code = '1')]"/>
                ...
                <!--<xsl:if test="not(marc:record/marc:datafield[@tag = ('100', '110', '111', '600', '610', '611', '700', '710', '711') and not(marc:subfield[@code='t']) and marc:subfield[@code='a'] and marc:subfield[@code = '1']])">
                        <xsl:copy-of select="bib:printasline('Alle agentinnførsler har $1 med URI', $format)"/>
                </xsl:if>-->
